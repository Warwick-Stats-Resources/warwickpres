...
# to remove logo from title slide, comment out the `data-background-image` line 
...
# To set up offline fonts:
# 1. render once (with eval: false) so that "offline_fonts.R" appears in root directory
# 2. set `eval: true` in this chunk and render once to set up offline fonts
# 3. then set back to false (or delete chunk)
source("offline_fonts.R")
```
...
but they are not made available by default.

To use them render this presentation once, so that `"offline_fonts.R"` appears in root directory.

Then either:

- call `source("offline_fonts.R")` once in the R console

or: 

- set `eval: true` in the first chunk of this presentation (with `#| label: offline-fonts`) and run once, then set back to `eval: false` (or delete chunk).
